...
void ConnectEdge();
void SendStatusLightThingsBoard();
void SendStatusLightEdge();
void OTA();
...
int RSSI;
String RSSIString;
uint32_t LightCurrent = 0;
uint32_t LightPrevious = 0;
uint32_t Timee = 0;
uint32_t TimeeE = 0;
...
void HandleProvisionRespone(const char* topic, byte* payload, unsigned int length) { 
  ...
  StaticJsonDocument<200> data3;
  StaticJsonDocument<200> data4;
  if (strcmp(topic, "/provision/response") == 0) {
    ...
    if (digitalRead(ControlLight) == HIGH) {
    ...
    /*char MessageJson3[length + 1];
    strncpy(MessageJson3, (char*)payload, length);
    Serial.println("Chuoi JSON nhan duoc la:");
    Serial.println(MessageJson3);
    MessageJson3[length] = '\0';
    DeserializationError error = deserializeJson (data3, MessageJson3);
    if(error){  
    Serial.println("Error deserialize Json");
    return;
    }
    boolean Attribute = data3["lighting"];
    if(Attribute == true) {
        digitalWrite(ControlLight, HIGH);
        StatusLight  = "ON";
        Send = 1;
  ...
  } else if (strcmp(topic, "v1/devices/me/rpc/response/1") == 0) {
    char MessageJson4[length + 1];
    strncpy(MessageJson4, (char*)payload, length);
    Serial.println("Chuoi JSON nhan duoc la:");
    Serial.println(MessageJson4);
    MessageJson4[length] = '\0';
    DeserializationError error = deserializeJson (data4, MessageJson4);
    if(error){  
    Serial.println("Error deserialize Json");
    return;
    }
    String Method = data4["method"].as<String>();
    LightCurrent = data4["light"];
...
void ReconnectThingsBoardDemo() {
 ...
 int Attempt = 1;
  ...
  if(mqttClient.connected()) {
    ...
    digitalWrite(LedGreen, HIGH);
    while (!mqttClient.subscribe("v1/devices/me/attributes")){
      delay(500);
    }
    Serial.println("Subscribe Attribute topic Done");
    while (!mqttClient.subscribe("v1/devices/me/rpc/response/1")){
      delay(500);
    }
    Serial.println("Subscribe RPC Response topic Done");
    ...
    Serial.println("Publish Infor WiFi Done");
    String Request = "{\"method\":\"" + String("GetButton")+ "\"" +",\"params\":" + "{}" + "}";
    Serial.println(Request);
    char RequestArray[100];
    Request.toCharArray(RequestArray, Request.length()+1);
    while ((mqttClient.connected()) && (mqttClient.publish("v1/devices/me/rpc/request/1", RequestArray))) {
      //Timee = millis();
      if (millis() - Timee > 10000) {
        Timee = millis();
        RSSI = WiFi.RSSI();
        RSSIString = "{\"RSSI\":" + String(RSSI) +"}";
      ...
      if (Send == 1) {
      Send = 0;
      String StatusLightString = "{\"Status Light\":" + String(StatusLight) + "}";
      StatusLightString.toCharArray(StatusLightArray, StatusLightString.length()+1);
      SendStatusLightThingsBoard();
      }
    }
...
void SendStatusLightThingsBoard() {
  if ((mqttClient.connected()) && (mqttClient.publish("v1/devices/me/telemetry", StatusLightArray))) {
    mqttClient.loop();
    Serial.println("Sent Status Light to ThingsBoardDemo Success");
  } else {
    Serial.println("Sent Status Light to ThingsBoardDemo Failure");
    digitalWrite(LedGreen, LOW);
    mqttClient.disconnect();
    Serial.println("Disconnect ThingsBoardDemo");
    return;
...
void ConnectEdge() {
 ...
 int Attempt = 1;
  ...
  if(mqttClient.connected()) {
    ...
    Serial.println("Publish Infor WiFi Done");
    String RequestE = "{\"method\":\"" + String("GetButton") +"\"" + ",\"params\":" + "{}" + "}";
    char RequestArrayE[100];
    RequestE.toCharArray(RequestArrayE, RequestE.length()+1);
    while ((mqttClient.connected()) && (mqttClient.publish("v1/devices/me/rpc/request/1", RequestArrayE))) {
      if (millis() - TimeeE > 10000) {
        TimeeE = millis();
        RSSI = WiFi.RSSI();
        RSSIString = "{\"RSSI\":" + String(RSSI) +"}";
        RSSIString.toCharArray(RSSIArray, RSSIString.length()+1);
      ...
      while(millis() - TimeE < 3000) {
        mqttClient.loop();
      }
      if (Send == 1) {
      Send = 0;
      String StatusLightStringE = "{\"Status Light\":" + String(StatusLight) + "}";
      StatusLightStringE.toCharArray(StatusLightArray, StatusLightStringE.length()+1);
      SendStatusLightEdge();
      }
    }
  } else {
...
void SendStatusLightEdge() {
  if ((mqttClient.connected()) && (mqttClient.publish("v1/devices/me/telemetry", StatusLightArray))) {
    mqttClient.loop();
    Serial.println("Sent Status Light to Edge Success");
  } else {
    Serial.println("Sent Status Light to Edge Failure");
    digitalWrite(LedRed, LOW);
    mqttClient.disconnect();
    Serial.println("Disconnect Edge");
    return;
