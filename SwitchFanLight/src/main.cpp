...
void IRAM_ATTR ChangeLight();
void IRAM_ATTR ChangeFan();
void DetermineLightFan();
...
void ConnectEdge();
void ReadLightFan();
void SendLightFanThingsBoard();
void SendLightFanEdge();
void OTA();
...
#define DeviceID "Switch"
#define Pass "123"
#define LedRed 16
#define LedGreen 17
#define ButtonLight 22
#define ButtonFan 21
#define Button 18
...
volatile int Change;
volatile uint32_t Light = 0;
volatile uint32_t Fan = 0;
volatile int Toggle;
...
void setup() {
  ...
  attachInterrupt(ButtonLight, ChangeLight, FALLING);
  attachInterrupt(ButtonFan, ChangeFan, FALLING);
}
...
void IRAM_ATTR ChangeLight() {
  portENTER_CRITICAL_ISR(&mux);
  //int State = HIGH;
  while (digitalRead(ButtonLight) != HIGH) {
    uint32_t StartTime = millis();
    while (millis() - StartTime <50) {
    }
  }
  Serial.println("Tonggle Light");
  Light = Light + 1;
  Toggle = 1;
  portEXIT_CRITICAL_ISR(&mux);
}
...
void IRAM_ATTR ChangeFan() {
  portENTER_CRITICAL_ISR(&mux);
  //int State = HIGH;
  while (digitalRead(ButtonFan) != HIGH) {
    uint32_t StartTime = millis();
    while (millis() - StartTime <50) {
    }
  }
  Serial.println("Tonggle Fan");
  Fan = Fan + 1;
  Toggle = 1;
  portEXIT_CRITICAL_ISR(&mux);
}
...
void DetermineLightFan() {
  Serial.println("Get value Light and Fan from Flash");
  preferences.begin("Switch", false);
  Light = preferences.getInt("Light", 0);
  Fan = preferences.getInt("Fan", 0);
  preferences.end();
  Serial.println("Gia tri cua Light la: ");
  Serial.println(Light);
  Serial.println("Gia tri cua Fan la: ");
  Serial.println(Fan);
}
...
void ReadLightFan() {
  ...
  //String RSSIString = "{\"RSSI\":" + String(RSSI) +"}";
  //RSSIString.toCharArray(RSSIArray, RSSIString.length()+1);
  String LighttFann = "{\"togglerLight\":" + String(Light) + ",\"togglerFan\":" + String(Fan) + "}";
  LighttFann.toCharArray(LightFanArray, LighttFann.length()+1);
}



void SendLightFanThingsBoard() {
  if ((mqttClient.connected()) && (mqttClient.publish("v1/devices/me/telemetry", LightFanArray))) {
    preferences.begin("Switch", false);
    preferences.putInt("Light", Light);
    preferences.putInt("Fan", Fan);
    preferences.end();
    mqttClient.loop();
    Serial.println("Sent Light and Fan to ThingsBoardDemo Success");
  } else {
    preferences.begin("Switch", false);
    preferences.putInt("Light", Light);
    preferences.putInt("Fan", Fan);
    preferences.end();
    Serial.println("Sent Light and Fan to ThingsBoardDemo Failure");
    digitalWrite(LedGreen, LOW);
    mqttClient.disconnect();
    Serial.println("Disconnect ThingsBoardDemo");
    return;
...
void SendLightFanEdge() {
  if ((mqttClient.connected()) && (mqttClient.publish("v1/devices/me/telemetry", LightFanArray))) {
    ...
    preferences.end();
    mqttClient.loop();
    Serial.println("Sent Light and Fan to Edge Success");
  } else {
    preferences.begin("Switch", false);
    preferences.putInt("Light", Light);
    preferences.putInt("Fan", Fan);
    preferences.end();
    Serial.println("Sent Light and Fan to Edge Failure");
    digitalWrite(LedRed, LOW);
    mqttClient.disconnect();
    Serial.println("Disconnect Edge");
    return;
...
void OTA() {
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "text/plain", "tren 3.2 Web, co len");
  });
